...
func _save_data(node: Node) -> void:
	...
	print("Saving rock -> Mined:", is_chopped, "| Mined on day:", chopped_on_day, "| Damage:", current_damage)
...
func _load_data(source_node: Node) -> void:
	...
	if tree == null:
		print("Rock not found:", node_path)
		return
	...
	if is_chopped:
		...
		print("Restoring rock as mined (Mined on day: ", chopped_on_day, ")")
	else:
		tree.show()
		if tree.has_method("enable_tree_physics"):
			tree.enable_tree_physics()
		print("Restoring normal rock")

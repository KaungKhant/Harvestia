...
@onready var carrot: Button = $MarginContainer/HBoxContainer/Carrot
@onready var pumpkin: Button = $MarginContainer/HBoxContainer/Pumpkin
...
@onready var pumpkin_label: Label = $MarginContainer/HBoxContainer/Pumpkin/PumpkinLabel
...
func _ready() -> void:
    ...
    pumpkin.focus_mode = Control.FOCUS_NONE
...
func on_inventory_changed() -> void:
    ...
    var carrot_count = inventory.get("carrot_seeds", 0)
    # Update the label's text visually, keeping the button's image clean!
    carrot_label.text = str(carrot_count)
    
    if carrot_count > 0:
        carrot.disabled = false
        carrot.focus_mode = Control.FOCUS_ALL
    else:
        carrot.disabled = true
        carrot.focus_mode = Control.FOCUS_NONE
    ...
    var pumpkin_count = inventory.get("pumpkin_seeds", 0)
    # Update the label's text visually, keeping the button's image clean!
    pumpkin_label.text = str(pumpkin_count)
    
    if pumpkin_count > 0:
        pumpkin.disabled = false
        pumpkin.focus_mode = Control.FOCUS_ALL
    else:
        pumpkin.disabled = true
        pumpkin.focus_mode = Control.FOCUS_NONE
        if ToolManager.selected_tool == DataTypes.Tools.PlantPumpkin:
            ToolManager.select_tool(DataTypes.Tools.None)
...
func _on_global_tool_selected(active_tool: DataTypes.Tools) -> void:
    ...
    match active_tool:
        ...
        DataTypes.Tools.PlantCarrot: carrot.modulate = Color(1.5, 1.5, 1.5, 1.0)
        DataTypes.Tools.PlantPumpkin: pumpkin.modulate = Color(1.5, 1.5, 1.5, 1.0)
...
func _on_carrot_pressed() -> void: ToolManager.select_tool(DataTypes.Tools.PlantCarrot)
func _on_pumpkin_pressed() -> void: ToolManager.select_tool(DataTypes.Tools.PlantPumpkin)

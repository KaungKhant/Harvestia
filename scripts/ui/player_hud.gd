...
func update_ui():
    level_label.text = "Lv. %d" % PlayerProgressManager.player_level
    ...
    gold_label.text = "🪙 %d" % PlayerProgressManager.gold


func _on_exp_changed(current_exp, required_exp):
    exp_bar.max_value = required_exp
    var tween = create_tween()
    tween.tween_property(exp_bar, "value", current_exp, 0.25)

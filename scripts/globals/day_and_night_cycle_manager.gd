...
# Game_MINUTES_DURATION represents how many radians pass per 1 real-world minute
...
var current_day: int = -1 # Changed to -1 so Day 1 properly triggers time_tick_day() on setup
...
func recalculate_time() -> void:
    ...
    var hour: int = int(current_days_minutes / MINUTES_PER_HOUR)
    var minute: int = int(current_days_minutes % MINUTES_PER_HOUR)
